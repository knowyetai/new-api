...
验收中管理员测试会话返回 401，重新登录恢复；密集重复认证操作还触发原生每 IP 20 次/20 分钟限制，`/api/oauth/state` 返回 429。未关闭或清除限流，等待窗口自然结束后，peilong 通过真实 SSO 重新登录再次验证通过。aibrain 首次检查误按 `user.username` 断言，按现有 `user.name` 响应契约修正验收脚本后通过；本轮未为这些测试环境/脚本问题修改业务代码。
