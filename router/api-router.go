...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		billingUnits.Use(middleware.UserAuth())
		{
			...
			billingUnits.POST("/self-service", middleware.CriticalRateLimit(), controller.CreateSelfServiceTeam)
			billingUnits.DELETE("/self/membership", controller.LeaveMyBillingTeam)
			billingUnits.PUT("/self/preference", controller.UpdateTeamBillingPreference)
			billingUnits.POST("/:id/select", controller.SelectOwnedBillingTeam)
			billingUnits.POST("/:id/invitations", middleware.CriticalRateLimit(), controller.CreateTeamInvitation)
			billingUnits.GET("/:id/invitations", controller.ListTeamInvitations)
			billingUnits.GET("/:id/usage-summary", controller.GetTeamUsageSummary)
			billingUnits.POST("/:id/topups/quote", controller.QuoteTeamEpay)
			billingUnits.POST("/:id/topups", middleware.CriticalRateLimit(), controller.RequestTeamEpay)
			billingUnits.GET("/:id/topups", controller.GetTeamTopups)
		...
		teamInvitations := apiRouter.Group("/billing-unit-invitations", middleware.UserAuth())
		teamInvitations.GET("/self", controller.ListTeamInvitations)
		teamInvitations.POST("/:invitation_id/:action", middleware.CriticalRateLimit(), controller.RespondTeamInvitation)
